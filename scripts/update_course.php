...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $editClassroom = $conn->real_escape_string(trim($_POST['editClassroom']));
    $editYear = $conn->real_escape_string(trim($_POST['editYear']));

    // Prepare the SQL statement
    $sql = "UPDATE `courses` SET `name` = ?, `department` = ?, `classroom` = ?, `year` = ? WHERE `id` = ?"; 

    if ($stmt = $conn->prepare($sql)) {
        // Bind parameters to the prepared statement
        $stmt->bind_param("ssssi", $name, $editDepartment, $editClassroom, $editYear, $id);
        ...
        if ($stmt->execute()) {
            ...
            if ($stmt->affected_rows > 0) {
                ...
                echo json_encode(['status' => 'info', 'message' => 'No changes made to the Course.']);
            }
        } else {
            // Handle errors during execution
            echo json_encode(['status' => 'error', 'message' => 'Could not update Course.']);
        }
