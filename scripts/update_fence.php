...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $class = $conn->real_escape_string(trim($_POST['editClass']));
    $A = $conn->real_escape_string(trim($_POST['editA']));
    $B = $conn->real_escape_string(trim($_POST['editB']));
    $C = $conn->real_escape_string(trim($_POST['editC']));
    $D = $conn->real_escape_string(trim($_POST['editD']));


    // Prepare the SQL statement
    $sql = "UPDATE `geo_fence` SET `class` = ?, `A` = ?, `B` = ?, `C` = ?,  `D` = ? WHERE `id` = ?";

    if ($stmt = $conn->prepare($sql)) {
        // Bind parameters to the prepared statement
        $stmt->bind_param("sssssi", $class, $A, $B, $C, $D, $id);

        // Attempt to execute the prepared statement
        if ($stmt->execute()) {
            // Check if the update was successful
            if ($stmt->affected_rows > 0) {
                echo json_encode(['status' => 'success', 'message' => 'Geo Fence updated successfully.']);
            } else {
